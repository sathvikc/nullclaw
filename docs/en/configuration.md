...
- `block_medium_risk_commands` (default: `true`): blocks network/transfer commands such as `curl`, `wget`, `nc`, `scp`, `ftp`, `telnet`. Set to `false` to allow these while still blocking high-risk commands.
